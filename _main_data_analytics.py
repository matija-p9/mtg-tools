from data_analytics import data_functions as df
import openpyxl

EXCEL_FILE = openpyxl.load_workbook("./reference/MTG Liga - 2024 #1.xlsx")
DATE_RANGE = ["2024-06-05", "2024-04-24"]
...
decks, meta_percentage, meta_ratio = df.metagame(EXCEL_FILE, date_list)
winrate_ratio = {}
i = 0
...
# Function call to process data.
winners, losers = df.player_wr(EXCEL_FILE, date_list)
winrate_percentage, wr_ratio_list = df.winrate(winners, losers)
winrate_ratio = {}
i = 0
# Convert data to dict for use.
...
winners, losers = df.deck_wr(EXCEL_FILE, date_list)
winrate_percentage, wr_ratio_list = df.winrate(winners, losers)
winrate_ratio = {}
i = 0
# Convert data to dict for use.
...
for deck in winrate_percentage:
    ...
    matrix_deck = df.matchup_matrix(EXCEL_FILE, date_list, deck)
